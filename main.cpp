...
bool TodoList::formatTaskId() {
    ...
    for (auto i = listTasks.begin(); i != listTasks.end(); ++i) {
        i->id = lastId++;
        if (i->deadline == 0)  i->deadline = currentTime;
        if (i->createdAt == 0) i->createdAt = currentTime;
    }
    return true;
}
...
void TodoList::printAllTasks() {
    ...
    cout << "╟────┼──────────────────────────────────────────────╢\n";
    for (auto i = listTasks.begin(); i != listTasks.end(); ++i) {
        cout << "║ " << setw(2) << setfill('0') << i->id << " │ "
        << setfill(' ') << setw(29) << left << truncStr(i->title, 29)
        << " " << right << setw(14) << getTanggal(i->deadline) << " ║\n";
    }
    cout << "╚════╧══════════════════════════════════════════════╝\n";
}
